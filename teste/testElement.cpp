...
TEST(ElementCreationTest, ElementCreation) {
	...
	EXPECT_EQ("Lifeline", element.getType());
}
...
TEST(ElementCreationTest, ElementCreation2) {
	...
	EXPECT_EQ("Optional", element.getType());
}
...
TEST(LifelineElementXMLTest, TransitionXML3) {
	Element element("shonbs", 1);

	EXPECT_EQ("<Lifeline name=\"shonbs\"/>", element.toXML());
}